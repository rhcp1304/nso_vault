...
# The helper functions for this section should also be moved to a services/utils file to avoid code duplication and improve organization. For this example, I'll keep them here.

# --- Configuration for PPT Uploads ---
...
def start_video_processing(request):
    """
    Handles the POST request from the form and starts the video
    processing in a Celery background task.
    """
    google_drive_folder_id = request.POST.get('google_drive_folder_id')
    if not google_drive_folder_id:
        return JsonResponse({'status': 'error', 'message': 'Missing Google Drive Folder ID.'}, status=400)

    # Run the main processing logic in a new Celery task
    task = process_video_task.delay(google_drive_folder_id)

    return JsonResponse({
        'status': 'success',
        'message': 'Processing started. Check the server console for progress updates.',
        'task_id': task.id
    })